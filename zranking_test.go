...
func TestZRanking(t *testing.T) {
	...
	list, err := zr.GetRankingList(ctx, 3, true)
	require.Nil(t, err)
	...
	list, err = zr.GetRankingList(ctx, 2, true)
	require.Nil(t, err)
	require.Len(t, list, 2)
	require.Equal(t, uid-1, list[0].UID)
	require.Equal(t, uid, list[1].UID)

	list, err = zr.GetRankingList(ctx, 4, true)
	require.Nil(t, err)
	require.Len(t, list, 3)

	list, err = zr.GetRankingList(ctx, 0, true)
	require.Nil(t, err)
	require.Len(t, list, 3)

	list, err = zr.GetRankingList(ctx, 0, false)
	require.Nil(t, err)
	require.Len(t, list, 3)
	require.Equal(t, uid-1, list[2].UID)
	require.Equal(t, uid, list[1].UID)
	...
	require.Len(t, list, 3)
	require.Equal(t, uid-1, list[0].UID)
	require.Equal(t, uid+1, list[1].UID)
	require.Equal(t, uid, list[2].UID)
	...
	ur, err = zr.GetUserRank(ctx, -1, true)
	require.Nil(t, err)
	require.Equal(t, int64(0), ur)
	uv, err = zr.GetUserVal(ctx, -1)
	require.Nil(t, err)
	require.Equal(t, int64(0), uv)
}
